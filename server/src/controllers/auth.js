...
export const register = async (req, res) => {
    try {
        ...
        const { error } = joi.object({name, mobile, password }).options({allowUnknown: true }).validate({...req.body,mobile:req.body.mobile})
        console.log('================');
        console.log(error);
...
export const logout = async (req, res) => {
    try { 
        ...
        if (!req.body.userId || ! req.body.token) return badRequest('userid or token are not define!', res)
