...
function Navigation() {
  ...
  return (                
    <div className=' w-full text-white  bg-secondary1 h-[50px] mb-[10px] ' >
      <div className='flex justify-start pl-1 f-full m-0   '>
      <div className='flex justify-center items-center    h-[50px]   font-medium text-sm'>
              <NavLink
                to={'/Home'}
        ...
        {categories.length > 0 && categories.map(el => { 
          return (
            <div key={el.code} className='flex justify-center items-center  h-[50px]   font-medium text-sm'>
              <NavLink
                to={el.code}
                className={({ isActive }) => isActive ? active : notactive}
              >{el.name}</NavLink>
