...
export default function Table({ columns, data }) {
  ...
  if (!data) data = []
    ...
    data = data.map(el => {
      return {
        ...el,
        startedAt: (!isExpired(el) || el.startedAt) ? el.startedAt : '-',
        expiredAt: (!isExpired(el) || el.expiredAt) ? el.expiredAt : '-',
        status: handlerStatus(el)
      }
    })
  }
                ...
                return (
                  <> 
                    ...
                    {cell.render("Cell").props.column.id === 'id'
                      && <td className={tableCss + ' px-2 py-1 h-[280px]'} {...cell.getCellProps()}>{cell.render("Cell")}</td>}

                    {cell.render("Cell").props.column.id === 'images'
                      && <td className={tableCss + ' px-2 py-1 w-[250px]'} {...cell.getCellProps()}><img src={cell.render("Cell").props.cell.value} className="h-[250px] w-[250px] object-cover" /></td>}

                    {cell.render("Cell").props.column.id === 'price'
                      && <td className={tableCss + ' px-2 py-1'} {...cell.getCellProps()}>{cell.render("Cell")}</td>}

                    {cell.render("Cell").props.column.id === 'detail'
                      && <td className={tableCss + ' px-2 py-1 flex items-start border-none'} {...cell.getCellProps()}><DetailCell PostId={row.cells[0]?.value} /></td>}

                    {cell.render("Cell").props.column.id === 'startedAt'
                      && <td className={tableCss + ' px-2 py-1'} {...cell.getCellProps()}>{handlerDateDisplay(cell.render("Cell").props.value)}</td>}

                    {cell.render("Cell").props.column.id === 'expiredAt'
                      && <td className={tableCss + ' px-2 py-1'} {...cell.getCellProps()}>{handlerDateDisplay(cell.render("Cell").props.value)}</td>}
                      
                    {cell.render("Cell").props.column.id === 'status'
                      && <td className={tableCss + ' px-2 py-1'} {...cell.getCellProps()}>{cell.render("Cell")}</td>}
